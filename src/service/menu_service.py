...
class MenuService:
    main_query = (
        ...
        .outerjoin(Submenus, Menus.id == Submenus.menu_id)
        ...
        menus = [
            ResponseMenu(
                **(
                    dict(
                        zip(self.col, row),
                        **{'submenus_count': row[-2], 'dishes_count': row[-1]}
                    )
                )
            )
            for row in rows
    ...
    async def get_menu_by_id(
        ...
        menu = dict(
            zip(self.col, row), **{'submenus_count': row[-2], 'dishes_count': row[-1]}
        )
        return ResponseMenu(**menu)

    async def add_menu(
        self, new_menu: BaseRequestModel, session: AsyncSession
    ) -> ResponseMenu:
    ...
    async def update_menu(
        self, target_menu_id: UUID, new_menu: BaseRequestModel, session: AsyncSession
    ) -> ResponseMenu:
        ...
        await self.check_exists_id_and_title(
            session, target_id=target_menu_id, title=new_menu.title
        )
        await session.execute(stmt)
        await session.commit()
        result = await session.execute(query)
    ...
    async def delete_menu(
        ...
        return ResponseMessage(status=True, message='The menu has been deleted')
