...
async def test_create_menu(async_client: AsyncClient, mocker: MockerFixture):
    mock = mocker.MagicMock(return_value=test_data["menu"]["id"])
    mocker.patch("uuid.uuid4", mock)
    response = await async_client.post("/menus", json=test_data["menu"])

    assert response.status_code == 201
    assert response.json()["id"] == test_data["menu"]["id"]


async def test_create_submenu(async_client: AsyncClient, mocker: MockerFixture):
    mock = mocker.MagicMock(return_value=test_data["submenu"]["id"])
    mocker.patch("uuid.uuid4", mock)
    response = await async_client.post(
        f"/menus/{test_data['menu']['id']}/submenus",
        json=test_data["submenu"])

    assert response.status_code == 201
    assert response.json()["id"] == test_data["submenu"]["id"]
...
async def test_create_dish1(async_client: AsyncClient, mocker: MockerFixture):
    ...
    mocker.patch("uuid.uuid4", mock)
    response = await async_client.post(
        f"/menus/{test_data['menu']['id']}/submenus/{test_data['submenu']['id']}/dishes",
        json=test_data["dish1"]
    )
...
async def test_menu_by_id(async_client: AsyncClient):
    response = await async_client.get(f"/menus/{test_data['menu']['id']}")
...
async def test_submenu_by_id(async_client: AsyncClient):
    response = await async_client.get(
        f"/menus/{test_data['menu']['id']}/submenus/{test_data['submenu']['id']}"
    )
...
async def test_delete_menu(async_client: AsyncClient):
    response = await async_client.delete(f"/menus/{test_data['menu']['id']}")

    assert response.status_code == 200


async def test_get_all_menus(async_client: AsyncClient):
    response = await async_client.get(f"/menus")
