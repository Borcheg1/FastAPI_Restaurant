...
async def test_get_all_menus_not_empty(async_client: AsyncClient,
                                       mocker: MockerFixture):
    mock = mocker.MagicMock(return_value=test_data["menu2"]["id"])
    mocker.patch("uuid.uuid4", mock)
    response = await async_client.post("/menus", json=test_data["menu2"])

    assert response.status_code == 201
    assert response.json()["id"] == test_data["menu2"]["id"]
    assert response.json()["title"] == test_data["menu2"]["title"]
    assert response.json()["description"] == test_data["menu2"]["description"]
    assert response.json()["submenus_count"] == 0
    assert response.json()["dishes_count"] == 0
    ...
    assert response2.json()[0]["id"] == test_data["menu1"]["id"]
    assert response2.json()[0]["title"] == test_data["menu1"]["title"]
    assert response2.json()[0]["description"] == test_data["menu1"]["description"]
    assert response2.json()[0]["submenus_count"] == 0
    assert response2.json()[0]["dishes_count"] == 0

    assert response2.json()[1]["id"] == test_data["menu2"]["id"]
    assert response2.json()[1]["title"] == test_data["menu2"]["title"]
    assert response2.json()[1]["description"] == test_data["menu2"]["description"]
...
async def test_patch_menu(async_client: AsyncClient):
    response = await async_client.patch(f"/menus/{test_data['menu1']['id']}",
                                        json=test_data["patch_menu1"])

    assert response.json()["id"] == test_data["menu1"]["id"]
    assert response.json()["title"] == test_data["patch_menu1"]["title"]
    assert response.json()["description"] == test_data["patch_menu1"]["description"]
    assert response.json()["submenus_count"] == 0
    assert response.json()["dishes_count"] == 0

    response2 = await async_client.patch(f"/menus/{test_data['menu2']['id']}",
                                        json=test_data["patch_menu2"])

    assert response2.json()["id"] == test_data["menu2"]["id"]
    assert response2.json()["title"] == test_data["patch_menu2"]["title"]
    assert response2.json()["description"] == test_data["patch_menu2"]["description"]
...
async def test_patch_menu_invalid_id(async_client: AsyncClient):
    response = await async_client.patch(f"/menus/{test_data['invalid_id']}",
                                        json=test_data["patch_menu1"])
    assert response.status_code == 404
    assert response.json()["detail"] == "menu not found"
...
async def test_get_all_menus_after_patch(async_client: AsyncClient):
    ...
    assert response.json()[0]["title"] == test_data["patch_menu1"]["title"]
    assert response.json()[0]["description"] == test_data["patch_menu1"]["description"]
    assert response.json()[0]["submenus_count"] == 0
    assert response.json()[0]["dishes_count"] == 0

    assert response.json()[1]["id"] == test_data["menu2"]["id"]
    assert response.json()[1]["title"] == test_data["patch_menu2"]["title"]
    assert response.json()[1]["description"] == test_data["patch_menu2"]["description"]
...
async def test_delete_menu(async_client: AsyncClient):
    response = await async_client.delete(f"/menus/{test_data['menu1']['id']}")
...
async def test_get_all_menus_after_delete(async_client: AsyncClient):
    ...
    assert response.json()[0]["id"] == test_data["menu2"]["id"]
    assert response.json()[0]["title"] == test_data["patch_menu2"]["title"]
    assert response.json()[0]["description"] == test_data["patch_menu2"]["description"]
    assert response.json()[0]["submenus_count"] == 0
    assert response.json()[0]["dishes_count"] == 0
...
async def test_delete_menu_by_invalid_id(async_client: AsyncClient):
    response = await async_client.get(f"/menus/{test_data['invalid_id']}")
