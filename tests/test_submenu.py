...
data: dict[str, Any] = {
    ...
    'patch_submenu1': {'title': 'patch_submenu1', 'description': 'patch_string1'},
    'patch_submenu2': {'title': 'patch_submenu2', 'description': 'patch_string2'},
    'invalid_id': '4d8b79de-e0cd-483e-9294-5425a5194492',
}

path = f"/menus/{data['menu1']['id']}/submenus"
...
@pytest.mark.parametrize('test_id', [data['menu1']['id'], data['invalid_id']])
...
async def test_post_submenu(async_client: AsyncClient, mocker: MockerFixture):
    ...
    mocker.patch('uuid.uuid4', mock)
    response = await async_client.post(f'{path}', json=data['submenu1'])
...
async def test_get_submenu_by_id(async_client: AsyncClient):
    response = await async_client.get(f"{path}/{data['submenu1']['id']}")
...
async def test_get_all_submenus_not_empty(
    async_client: AsyncClient, mocker: MockerFixture
):
    mock = mocker.MagicMock(return_value=data['submenu2']['id'])
    mocker.patch('uuid.uuid4', mock)
    response = await async_client.post(f'{path}', json=data['submenu2'])

    assert response.status_code == 201
    assert response.json()['id'] == data['submenu2']['id']
    assert response.json()['title'] == data['submenu2']['title']
    assert response.json()['description'] == data['submenu2']['description']
    assert response.json()['dishes_count'] == 0
    ...
    assert response2.json()[0]['id'] == data['submenu1']['id']
    assert response2.json()[0]['title'] == data['submenu1']['title']
    assert response2.json()[0]['description'] == data['submenu1']['description']
    assert response2.json()[0]['dishes_count'] == 0

    assert response2.json()[1]['id'] == data['submenu2']['id']
    assert response2.json()[1]['title'] == data['submenu2']['title']
    assert response2.json()[1]['description'] == data['submenu2']['description']
...
async def test_patch_submenu(async_client: AsyncClient):
    response = await async_client.patch(
        f"{path}/{data['submenu1']['id']}", json=data['patch_submenu1']
    )

    assert response.json()['id'] == data['submenu1']['id']
    assert response.json()['title'] == data['patch_submenu1']['title']
    assert response.json()['description'] == data['patch_submenu1']['description']
    assert response.json()['dishes_count'] == 0

    response2 = await async_client.patch(
        f"{path}/{data['submenu2']['id']}", json=data['patch_submenu2']
    )

    assert response2.json()['id'] == data['submenu2']['id']
    assert response2.json()['title'] == data['patch_submenu2']['title']
    assert response2.json()['description'] == data['patch_submenu2']['description']
    assert response2.json()['dishes_count'] == 0


async def test_patch_submenu_same_title(async_client: AsyncClient):
    response = await async_client.patch(
        f"{path}/{data['submenu1']['id']}", json=data['patch_submenu2']
    )
...
async def test_patch_submenu_invalid_id(async_client: AsyncClient):
    response = await async_client.patch(
        f"{path}/{data['invalid_id']}", json=data['patch_submenu1']
    )
    assert response.status_code == 404
    assert response.json()['detail'] == 'submenu not found'
...
async def test_get_all_submenus_after_patch(async_client: AsyncClient):
    ...
    assert response.json()[0]['title'] == data['patch_submenu1']['title']
    assert response.json()[0]['description'] == data['patch_submenu1']['description']
    assert response.json()[0]['dishes_count'] == 0

    assert response.json()[1]['id'] == data['submenu2']['id']
    assert response.json()[1]['title'] == data['patch_submenu2']['title']
    assert response.json()[1]['description'] == data['patch_submenu2']['description']
...
async def test_delete_menu(async_client: AsyncClient):
    response = await async_client.delete(f"{path}/{data['submenu1']['id']}")
...
async def test_get_all_submenus_after_delete_submenu(async_client: AsyncClient):
    ...
    assert response.json()[0]['id'] == data['submenu2']['id']
    assert response.json()[0]['title'] == data['patch_submenu2']['title']
    assert response.json()[0]['description'] == data['patch_submenu2']['description']
    assert response.json()[0]['dishes_count'] == 0
